      ...
      <div className="bg-white rounded-lg border border-gray-200 p-4 space-y-4">
        <div className="flex flex-wrap gap-4 items-end">
          <div>
            ...
            <select className="border border-gray-300 rounded px-3 py-1.5 text-sm bg-white" value={year} onChange={(e) => { setYear(Number(e.target.value)); setMembers([]); setPool(null); }}>
              {YEAR_OPTIONS.map((y) => <option key={y}>{y}</option>)}
            </select>
          </div>
          <div>
            <label className="block text-xs text-gray-500 mb-2">Pool Members (select ≥ 2)</label>
      ...
      {members.length > 0 && (
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
            <h3 className="text-sm font-semibold text-gray-700">Pool Members — Adjusted CB</h3>
            <div className={`text-sm font-semibold px-3 py-1 rounded-full ${poolSum >= 0 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
            ...
            <tbody className="divide-y divide-gray-100">
              {members.map((m) => (
                <tr key={m.shipId} className="hover:bg-gray-50">
                  <td className="px-4 py-3 font-mono font-medium text-blue-700">{m.shipId}</td>
                  <td className="px-4 py-3 font-mono">{m.cb ? m.cb.ghgIntensityActual.toFixed(4) : '—'}</td>
                  ...
                  <td className="px-4 py-3">
                    {m.cb && (
                      <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${m.cb.cbGco2eq >= 0 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
          ...
          <div className="px-4 py-3 border-t border-gray-100">
            {!isPoolValid && poolSum < 0 && (
              <p className="text-xs text-red-600 mb-2">Pool aggregate is negative. Add more surplus ships or remove deficit ships.</p>
            )}
            <button
      ...
      {pool && (
        <div className="bg-green-50 border border-green-200 rounded-lg p-4 space-y-3">
          <div className="flex items-center gap-2">
            <span className="text-green-700 font-semibold text-sm">Pool Created Successfully</span>
            <span className="text-xs text-green-600 font-mono">ID: {pool.id}</span>
          </div>
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-green-200 text-sm">
              <thead>
                <tr>
                  {['Ship ID', 'CB Before', 'CB After', 'Transfer'].map((h) => (
                    <th key={h} className="px-4 py-2 text-left text-xs font-medium text-green-700 uppercase">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-green-100">
                {pool.members.map((m) => (
                  <tr key={m.shipId}>
                    <td className="px-4 py-2 font-mono font-medium">{m.shipId}</td>
                    <td className={`px-4 py-2 font-mono ${m.cbBefore >= 0 ? 'text-green-700' : 'text-red-600'}`}>{fmt(m.cbBefore)}</td>
                    <td className={`px-4 py-2 font-mono font-semibold ${m.cbAfter >= 0 ? 'text-green-700' : 'text-red-600'}`}>{fmt(m.cbAfter)}</td>
